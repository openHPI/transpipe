typeof navigator === "object" && (function (global, factory) {
  ...
  const getMillis = value => Math.trunc(value % 1 * 1000); // Format time to UI friendly string
  ...
  function formatTime(time = 0, displayHours = false, inverted = false) {
    ...
    const formatMillis = value => String(value).padStart(3, '0'); // Breakdown to hours, mins, secs


    let hours = getHours(time);
    const mins = getMinutes(time);
    const secs = getSeconds(time);
    const millis = getMillis(time); // Do we need to display hours?
    ...
    return `${inverted && time > 0 ? '-' : ''}${hours}${format(mins)}:${format(secs)}:${formatMillis(millis)}`;
  }
